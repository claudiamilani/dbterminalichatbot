...
@section('content')
    <div class="row">
        @component('components.widget', ['size' => 8])
            @slot('title')
                @lang('DBT/documents.create.title')
            @endslot
            @slot('body')
                {!! Form::open(['route' => 'admin::dbt.documents.store','class' => 'form-horizontal', 'autocomplete' => 'off', 'files' => true]) !!}
                ...
                <div class="form-group required">
                    ...
                    <div class="col-md-9">
                        {!! Form::select('document_type_id', [], null, ['id' => 'document_type_id', 'class' => 'form-control', 'required']) !!}
                    </div>
                </div>
            @endslot
            @slot('footer')
