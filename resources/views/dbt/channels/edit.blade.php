...
@section('content')
    <div class="row">
        @component('components.widget', ['size' => 8])
            @slot('title')
                @lang('DBT/channels.edit.title')
            @endslot
            @slot('body')
                {!! Form::model($channel, ['method' => 'patch', 'route' => ['admin::dbt.channels.update', $channel->id],'class' => 'form-horizontal']) !!}
                <div class="form-group required">
                    <label for="name"
                           class="col-md-3 control-label"> @lang('DBT/channels.attributes.name')</label>
                    <div class="col-md-9">
