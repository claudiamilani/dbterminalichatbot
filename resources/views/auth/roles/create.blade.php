@extends('layouts.adminlte.template',['page_title' => trans('roles.create.title')])
@section('content')
    <div class="row">
        {!! Form::open(['route' => 'admin::roles.store','class' => 'form-horizontal']) !!}
        @component('components.widget', ['size' => 12])
            @slot('title')@lang('roles.create.title') @endslot
            @slot('body')
                ...
                <div class="form-group">
                    ...
                    <div class="col-md-10">
                        {!! Form::textarea('description', null, [ 'id' => 'description','class' => 'form-control vertical height-sm']) !!}
                    </div>
                </div>
                <div class="form-group">
                                        ...
                                        <div class="card">
                                            ...
                                            <ul class="list-group list-group-flush" id="list{{$type->id}}">
                                                @foreach ($type->permissions as $permission)
                                                    <li class="list-group-item">{!! Form::label('check', trans('permissions.attributes.'.$permission->label)) !!} {!! Form::checkbox('permissions[]', $permission->id, 0 , ['id'=> 'checks','class' => 'pull-right']) !!}</li>
                                                @endforeach
                                            </ul>
                                        </div>
                                    </div>
