...
class SiteResetPasswordController extends controller
{
    /**
     ...
     * @param $token
     *
     * Show the form frontend side to reset password providing the proper token
     *
     * @return Application|Factory|View|\Illuminate\Foundation\Application|\Illuminate\View\View
     */
    public function showResetForm(Request $request, $token = null)
    {
        return view('layouts.site.password-reset')->with(
    ...
    public function reset(Request $request)
    {
        ...
        if (!$check_user) {
            ...
            return redirect()->route(RoutingManager::pwdResetRoute(), $token)->with(['alerts' => [
        ...
        try {
            if (($driver->resetPassword($user, $request->all())) !== true) {
                return redirect()->back()->with(['alerts' => [
                    ['message' => trans('Errore temporaneo'), 'type' => 'danger']]])->onlyInput('user');
            }
        } catch (Exception $e) {
        ...
        return redirect()->route(RoutingManager::loginRoute(), ['auth_type' => $user->authType->id])->with(['alerts' => [
